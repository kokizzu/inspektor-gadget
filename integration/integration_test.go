...
func TestBindsnoop(t *testing.T) {
	...
	commands := []*command{
		bindsnoopCmd, // Start it.
		{
			name:           "Run pod which calls bind()",
			cmd:            "kubectl run --restart=Never --image=busybox -n test-ns test-pod -- sh -c 'while true; do nc -l -p 9090 -w 1; done'",
			expectedString: "pod/test-pod created\n",
		},
		waitUntilTestPodReady,
		{
			name: "Wait so bind() are called",
			cmd:  "sleep 15",
		},
		deleteTestPod,
		bindsnoopCmd, // Stop it.
	}
...
func TestExecsnoop(t *testing.T) {
	...
	commands := []*command{
		execsnoopCmd, // Start it.
		{
			name:           "Run pod which does a lot of exec",
			cmd:            "kubectl run --restart=Never --image=busybox -n test-ns test-pod -- sh -c 'while true; do date; done'",
			expectedString: "pod/test-pod created\n",
		},
		waitUntilTestPodReady,
		deleteTestPod,
		execsnoopCmd, // Stop it.
	}
...
func TestNetworkpolicy(t *testing.T) {
	commands := []*command{
		{
			name:           "Run test pod",
			cmd:            "kubectl run --restart=Never --image=busybox -n test-ns test-pod -- sh -c 'while true; do wget -q -O /dev/null https://kinvolk.io; done'",
			expectedRegexp: "pod/test-pod created",
		},
		waitUntilTestPodReady,
		{
...
func TestOpensnoop(t *testing.T) {
	...
	commands := []*command{
		opensnoopCmd, // Start it.
		{
			name:           "Run pod which calls open()",
			cmd:            "kubectl run --restart=Never --image=busybox -n test-ns test-pod -- sh -c 'while true; do whoami; done'",
			expectedRegexp: "pod/test-pod created",
		},
		waitUntilTestPodReady,
		deleteTestPod,
		opensnoopCmd, // Stop it.
	}
...
func TestTcpconnect(t *testing.T) {
	...
	commands := []*command{
		tcpconnectCmd, // Start it.
		{
			name:           "Run pod which opens TCP socket",
			cmd:            "kubectl run --restart=Never --image=busybox -n test-ns test-pod -- sh -c 'while true; do wget -q -O /dev/null -T 3 http://1.1.1.1; done'",
			expectedRegexp: "pod/test-pod created",
		},
		waitUntilTestPodReady,
		deleteTestPod,
		tcpconnectCmd, // Stop it.
	}
