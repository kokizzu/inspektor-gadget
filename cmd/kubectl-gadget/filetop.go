...
var filetopCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) >= 1 {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "%q is not a valid value for interval\n", args[0])
				os.Exit(1)
			}
		}
		...
		config := &utils.TraceConfig{
			...
			Parameters: map[string]string{
				"output_rows": strconv.Itoa(maxrows),
	...
	case utils.OutputModeColumns:
		...
		for idx, event := range stats {
			...
			fmt.Printf("%-16s %-16s %-16s %-16s %-7d %-16s %-6d %-6d %-7d %-7d %c %s\n",
				...
				event.Tid, event.Comm, event.Reads, event.Writes, event.ReadBytes/1024,
