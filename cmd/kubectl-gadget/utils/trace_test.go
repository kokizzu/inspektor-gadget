...
func TestPrintTraceFeedback(t *testing.T) {
	...
	printTraceFeedback(msgType, mock.Printf, m)
	expected := fmt.Sprintf("%s running trace on node \"node\": Err/Warn Message", msgType)
	...
	for node, msg := range m {
		expected = fmt.Sprintf("%s running trace on node \"%s\": %s", msgType, node, msg)
	...
	printTraceFeedback(msgType, mock.Printf, m)
	expected = fmt.Sprintf("%s running trace on all nodes: Err/Warn Message", msgType)
	if expected != string(mock.output) {
		t.Fatalf("%v != %v", string(mock.output), expected)
	}
}
