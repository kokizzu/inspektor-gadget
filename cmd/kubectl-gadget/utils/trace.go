...
func printTraceFeedback(msgType string, f func(format string, args ...interface{}), m map[string]string) {
	// Do not print `len(m)` times the same message if it's the same from all nodes
	if len(m) > 1 {
		value := getIdenticalValue(m)
		if value != "" {
			f("%s running trace on all nodes: %s", msgType, value)
			return
		}
	}

	for node, msg := range m {
		f("%s running trace on node %q: %s", msgType, node, msg)
	}
}
...
RetryLoop:
	for {
		...
		if successNodeCount == 0 {
			printTraceFeedback("Warning", contextLogger.Warningf, nodeWarnings)
		}

		// Print errors even if other nodes succeeded.
		printTraceFeedback("Error", contextLogger.Errorf, nodeErrors)

		if successNodeCount == 0 {
			deleteTraces(contextLogger, traceRestClient, traceID)
			contextLogger.Fatalf("Error running trace on nodes: None of them succeeded")
			contextLogger.Fatalf("Error running trace on all nodes: None of them succeeded")
		}
		break RetryLoop
	}
